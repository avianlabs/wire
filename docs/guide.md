...
func initializeBaz(ctx context.Context) (foobarbaz.Baz, error) {
    ...
    bar := foobarbaz.ProvideBar(
        foo,
    )
    baz, err := foobarbaz.ProvideBaz(
        ctx,
        bar,
    )
    if err != nil {
...
Note that unlike the original Google Wire, this fork formats provider calls
that take arguments with one argument per line, which keeps generated code
readable and diffs small as dependencies change.
